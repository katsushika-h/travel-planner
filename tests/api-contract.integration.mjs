...
const removedScheduleFields = ["startDateTime", "endDateTime", "dayIndex"];
function assertCanonicalObject(item) {
  for (const field of removedScheduleFields) assert.equal(Object.hasOwn(item, field), false, `${field} must not be returned`);
}
...
try {
  ...
  assert.deepEqual(await request("/api/travel-objects", "POST", { ...base, title: "Invalid content", cost: { amount: -1, currency: "USD" }, location: [] }), { status: 400, body: { error: "cost must have a non-negative numeric amount." } });
  ...
  assertCanonicalObject(unscheduled.body);
  ...
  assert.deepEqual([flexible.body.placementTime, flexible.body.startTime, flexible.body.endTime], ["09:15", null, null]);
  assertCanonicalObject(flexible.body);
  ...
  assert.deepEqual([fixed.body.date, fixed.body.endDate, fixed.body.startTime, fixed.body.endTime], ["2026-09-24", "2026-09-25", "23:30", "01:00"]);
  assertCanonicalObject(fixed.body);
  ...
  assert.deepEqual(await request(`/api/travel-objects/${fixed.body.id}`, "PATCH", { headerImage: "data:image/png;base64,AA", location: [] }), { status: 400, body: { error: "headerImage must be an http or https image URL." } });
  ...
  assert.deepEqual(await request(`/api/travel-objects/${fixed.body.id}`, "PATCH", { type: null }), { status: 400, body: { error: "type must be a non-empty string." } });
  ...
  assert.deepEqual([allDay.body.date, allDay.body.endDate, allDay.body.startTime, allDay.body.endTime], ["2026-09-25", "2026-09-26", null, null]);
  assertCanonicalObject(allDay.body);

  for (const field of removedScheduleFields) {
    const rejected = await request("/api/travel-objects", "POST", { ...base, title: "Removed field", [field]: null });
    assert.deepEqual(rejected, { status: 400, body: { error: `${field} is no longer supported; use canonical schedule fields.` } });
  }
  const moved = await request("/api/travel-objects", "POST", { ...base, title: "Moved", date: "2026-09-24", endDate: "2026-09-24", startTime: "09:00", endTime: "10:00", isAllDay: false });
  ...
  assert.deepEqual([canonicalPatch.body.endDate, canonicalPatch.body.endTime], ["2026-09-25", "02:00"]);
  assertCanonicalObject(canonicalPatch.body);
  ...
  for (const field of removedScheduleFields) {
    const rejected = await request(`/api/travel-objects/${moved.body.id}`, "PATCH", { [field]: null });
    assert.deepEqual(rejected, { status: 400, body: { error: `${field} is no longer supported; use canonical schedule fields.` } });
  }
  const canonicalMove = await request(`/api/travel-objects/${moved.body.id}`, "PATCH", { date: "2026-09-25", endDate: "2026-09-25" });
  assert.equal(canonicalMove.status, 200);
  assert.deepEqual([canonicalMove.body.date, canonicalMove.body.startTime, canonicalMove.body.endTime], ["2026-09-25", "09:00", "10:00"]);
  const afterCanonicalMove = await request(`/api/travel-objects?tripId=${tripId}`);
  for (const date of ["2026-09-24", "2026-09-25"]) {
    const orders = afterCanonicalMove.body.filter((item) => item.date === date).map((item) => item.dayOrder).sort((a, b) => a - b);
    assert.deepEqual(orders, Array.from({ length: orders.length }, (_, index) => index), `${date} order after canonical move`);
  }

  const canonicalTime = await request(`/api/travel-objects/${moved.body.id}`, "PATCH", { startTime: "12:00", endTime: "13:00" });
  assert.equal(canonicalTime.status, 200);
  assert.deepEqual([canonicalTime.body.date, canonicalTime.body.startTime, canonicalTime.body.endTime], ["2026-09-25", "12:00", "13:00"]);

  const canonicalUnschedule = await request(`/api/travel-objects/${flexible.body.id}`, "PATCH", { date: null });
  assert.equal(canonicalUnschedule.status, 200);
  assert.deepEqual([canonicalUnschedule.body.date, canonicalUnschedule.body.endDate, canonicalUnschedule.body.placementTime, canonicalUnschedule.body.dayOrder], [null, null, null, null]);
  const afterCanonicalUnschedule = await request(`/api/travel-objects?tripId=${tripId}`);
  const remainingOrders = afterCanonicalUnschedule.body.filter((item) => item.date === "2026-09-24").map((item) => item.dayOrder).sort((a, b) => a - b);
  assert.deepEqual(remainingOrders, Array.from({ length: remainingOrders.length }, (_, index) => index), "source order after canonical unschedule");
  ...
  list.body.forEach(assertCanonicalObject);
  const itemDetail = await request(`/api/travel-objects/${moved.body.id}`);
  assert.equal(itemDetail.status, 200);
  assertCanonicalObject(itemDetail.body);
  ...
  console.log("API contract fixture passed: GET, POST, PATCH, DELETE, canonical schedules, and removed-field rejection.");
} finally {
  if (tripId) {
