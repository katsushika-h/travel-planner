...
function DayCard({ item, date, timezone, color, onSelect }: { item: TravelObject; date: string; timezone: string; color: string; onSelect: (item: TravelObject) => void }) {
  ...
  return <button ref={setNodeRef} {...attributes} {...listeners} type="button" onClick={() => onSelect(item)} style={{ borderLeft: `4px solid ${color}`, ...(transform ? { transform: `translate3d(${transform.x}px,${transform.y}px,0)` } : {}) }} className={`w-full rounded-lg border bg-background p-3 text-left shadow-sm ${isDragging ? "opacity-40" : ""}`}><p className="flex items-center gap-1.5 text-sm font-medium"><GripVertical size={13} className="shrink-0 text-muted-foreground"/><span className="truncate">{item.title}</span></p><p className="mt-1 pl-5 text-xs text-muted-foreground">{item.isAllDay ? "All day" : `${dateParts(item.startDateTime, timezone).time}–${dateParts(item.endDateTime, timezone).time}`} · {item.type}</p>{place && <p className="mt-1 flex items-center gap-1 pl-5 text-xs text-muted-foreground"><MapPin size={11}/>{place}</p>}</button>;
}
export function TripDaysView({ trip, items, typeColors, onSelect, onMove, onCreateItem }: { trip: Trip; items: TravelObject[]; typeColors: Record<string, string>; onSelect: (item: TravelObject) => void; onMove: (item: TravelObject, date: string) => Promise<void>; onCreateItem: (date: string) => void }) {
  ...
  return <div className="flex min-h-0 flex-1 flex-col overflow-hidden rounded-xl border bg-stone-50 dark:bg-neutral-950"><header className="border-b bg-white px-4 py-3 dark:bg-neutral-900"><h2 className="text-sm font-semibold">Day view</h2><p className="text-xs text-muted-foreground">Move an item to another day to keep its time and change its date.</p></header><DndContext sensors={sensors} onDragEnd={onDragEnd}><div className="flex min-h-0 flex-1 items-start gap-3 overflow-x-auto p-4">{days.map((date, index) => <TripDayColumn key={date} date={date} index={index} timezone={trip.timezone} items={items.filter((item) => dateParts(item.startDateTime, trip.timezone).date <= date && dateParts(item.endDateTime, trip.timezone).date >= date).sort((a, b) => {
  ...
  })} colorMap={typeColors} onSelect={onSelect} onCreate={onCreateItem} />)}</div></DndContext></div>;
}
