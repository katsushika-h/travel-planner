...
export function MapView({ trip, items, typeColors, onSelect }: { trip: Trip; items: TravelObject[]; typeColors: Record<string, string>; onSelect: (item: TravelObject) => void }) {
  ...
  const visibleItems = useMemo(() => activeDates.size ? mappedItems.filter((item) => item.date && activeDates.has(item.date.slice(0, 10))) : mappedItems, [activeDates, mappedItems]);
