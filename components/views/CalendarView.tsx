...
import { ChevronLeft, ChevronRight, Clock, GripVertical, ChevronDown, MapPin, Plus } from "lucide-react";
...
function UnscheduledDrawer({ items, typeColors, selectedIds, primarySelectedId, onSelect, onAdd, expanded = false, onExpandedChange }: { items: TravelObject[]; typeColors: Record<string, string>; selectedIds: ReadonlySet<string>; primarySelectedId: string | null; onSelect: (item: TravelObject, additive?: boolean) => void; onAdd: () => void; expanded?: boolean; onExpandedChange?: (expanded: boolean) => void }) {
  const [open, setOpen] = useState(false);
  const closeTimer = useRef<ReturnType<typeof setTimeout> | null>(null);
  const { setNodeRef, isOver } = useDroppable({ id: "unscheduled" });
  const visible = open || expanded || isOver;
...
function FlexibleScheduleEvent({ item, date, color, hourHeight, lane, laneCount, selected, primary, onSelect }: { item: TravelObject; date: string; color: string; hourHeight: number; lane: number; laneCount: number; selected: boolean; primary: boolean; onSelect: (item: TravelObject, additive?: boolean) => void }) {
  const { attributes, listeners, setNodeRef, transform, isDragging } = useDraggable({ id: `flex:${date}:${item.id}`, data: { item } });
  const [hour, minute] = (item.placementTime ?? "09:00").split(":").map(Number);
  const style: CSSProperties = { top: ((hour * 60 + minute) / 60) * hourHeight, height: Math.max(12, hourHeight / 4), left: `calc(${(lane / laneCount) * 100}% + 4px)`, width: `calc(${100 / laneCount}% - 8px)`, borderLeftColor: color, ...(transform ? { transform: `translate3d(${transform.x}px,${transform.y}px,0)` } : {}) };
  return <button ref={setNodeRef} {...attributes} {...listeners} type="button" onClick={(event) => onSelect(item, event.shiftKey)} style={style} className={`absolute z-[2] flex items-center gap-1 overflow-hidden rounded border border-l-4 bg-white/95 px-1.5 text-left text-[10px] shadow-sm dark:bg-neutral-900/95 ${selectionRing(selected, primary)} ${isDragging ? "opacity-40" : ""}`}><GripVertical size={11} className="shrink-0 text-muted-foreground" /><span className="min-w-0 flex-1 truncate font-medium">{item.title}</span></button>;
}
...
function ScheduleColumn({ date, items, allDayItems, timezone, typeColors, width, hourHeight, selectedIds, primarySelectedId, onSelect, onCreateItem, onResize, onResizeStart }: { date: string; items: TravelObject[]; allDayItems: TravelObject[]; timezone: string; typeColors: Record<string, string>; width: number; hourHeight: number; selectedIds: ReadonlySet<string>; primarySelectedId: string | null; onSelect: (item: TravelObject, additive?: boolean) => void; onCreateItem: (date: string, time: string) => void; onResize: (item: TravelObject, endDate: string, endTime: string) => void; onResizeStart: (item: TravelObject, startDate: string, startTime: string) => void }) {
  const flexiblePlanItems = items.filter((item) => !item.isAllDay && !item.startTime && !item.endTime);
  const fixedItems = items.filter((item) => Boolean(item.startDateTime && item.endDateTime));
  const { setNodeRef, isOver } = useDroppable({ id: `day:${date}` });
...
function AgendaGap({ date, order, onCreate }: { date: string; order: number; onCreate: (date: string, order: number) => void }) {
  ...
  return <div ref={setNodeRef} className={`group flex h-9 w-full items-center gap-3 px-4 transition ${isOver ? "bg-emerald-50 dark:bg-emerald-950/30" : ""}`}><span className="w-20 shrink-0" /><span className={`h-px flex-1 border-t transition ${isOver ? "border-2 border-emerald-500" : "border-dotted border-transparent group-hover:border-stone-300 dark:group-hover:border-neutral-600"}`} /><button type="button" aria-label={`Add item at position ${order + 1}`} onClick={() => onCreate(date, order)} className={`flex items-center gap-1 text-[10px] font-medium transition ${isOver ? "text-emerald-700" : "text-transparent group-hover:text-stone-500 dark:group-hover:text-stone-400"}`}><Plus size={12} /> Add item</button></div>;
}

function DayAgendaCard({ item, number, timezone, color, selected, primary, onSelect }: { item: TravelObject; number: number; timezone: string; color: string; selected: boolean; primary: boolean; onSelect: (item: TravelObject, additive?: boolean) => void }) {
  ...
  const hasFixedTime = Boolean(item.startTime && item.endTime && !item.isAllDay);
  const details = [item.type, ...(item.tags ?? []).slice(0, 2)].join(" · ");
  return <div ref={setNodeRef} {...attributes} {...listeners} style={transform ? { transform: `translate3d(${transform.x}px,${transform.y}px,0)` } : undefined} className={`group relative flex items-stretch gap-3 ${isDragging ? "opacity-40" : ""}`}>
    <span className={`relative z-10 w-20 shrink-0 pt-4 text-right font-semibold ${hasFixedTime ? "text-lg leading-none text-stone-500 dark:text-stone-400" : "text-[11px] text-stone-400"}`}>{hasFixedTime ? item.startTime : item.isAllDay ? "All day" : ""}</span>
    {hasFixedTime && <span aria-hidden className="absolute left-24 right-0 top-7 border-t border-stone-300/80 dark:border-neutral-700" />}
    <article style={{ borderLeftColor: color }} className={`relative z-10 min-w-0 flex-1 overflow-hidden rounded-xl border border-l-4 bg-white shadow-sm transition hover:shadow-md dark:bg-neutral-900 ${selectionRing(selected, primary)}`}>{item.headerImage && <button type="button" aria-label={`Open ${item.title}`} onClick={(event) => onSelect(item, event.shiftKey)} className="block w-full"><img src={item.headerImage} alt="" className="aspect-[3/1] w-full object-cover" /></button>}<div className="flex"><div className="flex w-14 shrink-0 items-start justify-center border-r bg-stone-50 px-1 py-4 dark:bg-neutral-800/60"><span className="grid size-7 place-items-center rounded-full text-xs font-bold text-white" style={{ backgroundColor: color }}>{number}</span></div><button type="button" onClick={(event) => onSelect(item, event.shiftKey)} className="min-w-0 flex-1 px-4 py-3 text-left"><div className="flex items-start gap-2"><GripVertical size={15} className="mt-0.5 shrink-0 text-muted-foreground opacity-50 group-hover:opacity-100" /><div className="min-w-0 flex-1"><p className="truncate text-sm font-semibold">{item.title}</p>{details && <p className="mt-1 text-xs text-muted-foreground">{details}</p>}{location && <p className="mt-2 flex items-center gap-1.5 truncate text-xs text-muted-foreground"><MapPin size={12} className="shrink-0" />{location}</p>}{item.notes && <p className="mt-2 line-clamp-2 text-xs leading-relaxed text-stone-500 dark:text-stone-400">{item.notes}</p>}</div></div></button></div></article>
  </div>;
}
...
function DayAgenda({ date, items, trip, typeColors, selectedIds, primarySelectedId, onSelect, onCreateItem }: { date: string; items: TravelObject[]; trip: Trip; typeColors: Record<string, string>; selectedIds: ReadonlySet<string>; primarySelectedId: string | null; onSelect: (item: TravelObject, additive?: boolean) => void; onCreateItem: (date: string, order: number) => void }) {
  ...
  const isWithinTrip = date >= tripStart && date <= tripEnd;
  const dayIndex = Math.floor((Date.parse(`${date}T00:00:00Z`) - Date.parse(`${tripStart}T00:00:00Z`)) / 86400000) + 1;
  const dayItems = isWithinTrip ? items.filter((item) => Boolean(item.date && item.date.slice(0, 10) <= date && (item.endDate?.slice(0, 10) ?? item.date.slice(0, 10)) >= date)) : [];
  const sortOrder = (a: TravelObject, b: TravelObject) => (a.dayOrder ?? Number.MAX_SAFE_INTEGER) - (b.dayOrder ?? Number.MAX_SAFE_INTEGER) || a.createdAt.localeCompare(b.createdAt);
  const allDay = dayItems.filter((item) => item.isAllDay).sort(sortOrder);
  const flexible = dayItems.filter((item) => !item.isAllDay && !item.startTime && !item.endTime).sort((a, b) => sortOrder(a, b) || (a.placementTime ?? "").localeCompare(b.placementTime ?? ""));
  const fixed = dayItems.filter((item) => item.startTime && item.endTime && !item.isAllDay).sort((a, b) => a.startTime!.localeCompare(b.startTime!));
  const agenda = [...allDay, ...fixed, ...flexible];
  return <div className="min-h-0 flex-1 overflow-y-auto bg-stone-50/60 dark:bg-neutral-950"><div className="mx-auto w-full max-w-4xl px-3 py-5 sm:px-7"><div className="mb-4 flex items-end justify-between gap-3"><div><p className="text-xs font-medium text-muted-foreground">{isWithinTrip ? `Day ${dayIndex}` : "Outside trip dates"}</p><h2 className="text-xl font-semibold">{formatTripDate(`${date}T12:00:00Z`, trip.timezone, { weekday: "long", month: "long", day: "numeric" })}</h2></div><p className="text-xs text-muted-foreground">{agenda.length} {agenda.length === 1 ? "item" : "items"}</p></div>{isWithinTrip ? <div className="space-y-0">{agenda.map((item, index) => <div key={item.id}><AgendaGap date={date} order={index} onCreate={onCreateItem} /><DayAgendaCard item={item} number={index + 1} timezone={trip.timezone} color={typeColor(item.type, typeColors)} selected={selectedIds.has(item.id)} primary={primarySelectedId === item.id} onSelect={onSelect} /></div>)}<AgendaGap date={date} order={agenda.length} onCreate={onCreateItem} />{agenda.length === 0 && <div className="grid min-h-40 place-items-center rounded-xl border border-dashed"><Button type="button" variant="outline" size="sm" onClick={() => onCreateItem(date, 0)}><Plus size={14} /> Add item</Button></div>}</div> : <div className="grid min-h-40 place-items-center rounded-xl border border-dashed px-5 text-center text-sm text-muted-foreground">This date is outside the trip, so it has no itinerary items.</div>}</div></div>;
}

export function CalendarView({ trip, items, typeColors = {}, selectedIds, primarySelectedId, onSelect, onSelectForDrag, onMove, onPlaceFlexible, onUnschedule, onResize, onResizeStart, onFlexibleDrop = async (item, date, order, clearTime, placementTime) => { await api.reorderObjects({ tripId: trip.id, objectId: item.id, date, dayOrder: order, clearTime, placementTime }); window.location.reload(); }, onCreateItem }: { trip: Trip; items: TravelObject[]; typeColors?: Record<string, string>; selectedIds: ReadonlySet<string>; primarySelectedId: string | null; onSelect: (item: TravelObject, additive?: boolean) => void; onSelectForDrag: (item: TravelObject) => void; onMove: (item: TravelObject, date: string, time?: string) => Promise<void>; onPlaceFlexible: (item: TravelObject, date: string, placementTime: string) => Promise<void>; onUnschedule: (item: TravelObject) => Promise<void>; onResize: (item: TravelObject, endDate: string, endTime: string) => Promise<void>; onResizeStart: (item: TravelObject, startDate: string, startTime: string) => Promise<void>; onFlexibleDrop?: (item: TravelObject, date: string, order: number, clearTime: boolean, placementTime?: string) => Promise<void>; onCreateItem: (date?: string, time?: string, type?: string, title?: string, dayOrder?: number) => void }) {
  ...
  const unscheduledItems = items.filter((item) => item.date === null);
  const tripEnd = trip.endDate.slice(0, 10);
  const dayIndexAtDate = (date: string) => date < start || date > tripEnd ? null : Math.floor((Date.parse(`${date}T00:00:00Z`) - Date.parse(`${start}T00:00:00Z`)) / 86400000) + 1;
  const flexibleItems = (date: string) => { const dayIndex = dayIndexAtDate(date); return dayIndex === null ? [] : items.filter((item) => item.date?.slice(0, 10) === date && !item.isAllDay && !item.startTime && !item.endTime).sort((a, b) => (a.placementTime ?? "").localeCompare(b.placementTime ?? "") || (a.dayOrder ?? Number.MAX_SAFE_INTEGER) - (b.dayOrder ?? Number.MAX_SAFE_INTEGER) || a.createdAt.localeCompare(b.createdAt)); };
  ...
  const weekStart = sundayOf(focusDate); const weekDates = Array.from({ length: 7 }, (_, index) => shiftDate(weekStart, index));
  ...
  useEffect(() => {
    const wasWeek = previousModeRef.current === "week";
    previousModeRef.current = mode;
    if (!wasWeek || mode !== "day" || !primarySelectedId) return;
    const selected = items.find((item) => item.id === primarySelectedId);
    const selectedDate = selected?.date?.slice(0, 10) ?? (selected?.startDateTime ? dateParts(selected.startDateTime, trip.timezone).date : null);
    if (!selectedDate) return;
    const frame = requestAnimationFrame(() => setFocusDate(selectedDate));
    return () => cancelAnimationFrame(frame);
  }, [items, mode, primarySelectedId, trip.timezone]);
  function finishDrag() { setActiveItem(null); requestAnimationFrame(() => { suppressClickRef.current = false; }); }
  async function handleDragEnd(event: DragEndEvent) {
    finishDrag();
    const target = String(event.over?.id ?? "");
    const item = (event.active.data.current as { item?: TravelObject } | undefined)?.item ?? items.find((candidate) => candidate.id === String(event.active.id));
    if (!item) return;
    if (target === "unscheduled") { if (item.date !== null) await onUnschedule(item); return; }
    const targetDate = target.startsWith("day-list:") ? target.split(":")[1] : target.startsWith("day:") ? target.slice(4) : null;
    if (item.isAllDay) { if (targetDate) await onMove(item, targetDate); return; }
    if (target.startsWith("day-list:")) {
      const [, date, rawOrder] = target.split(":");
      if (date) await onFlexibleDrop(item, date, Number(rawOrder) || 0, Boolean(item.startTime || item.endTime), item.placementTime ?? "09:00");
      return;
    }
    if (!target.startsWith("day:")) return;
    const destination = target.slice(4);
    const isFlexible = Boolean(!item.startTime && !item.endTime);
    const original = item.startDateTime ? dateParts(item.startDateTime, trip.timezone) : null;
    let nextDate = destination;
    let nextTime: string | undefined;
    if (mode !== "month" && original) {
      const [hour, minute] = original.time.split(":").map(Number);
      const total = hour * 60 + minute + Math.round((event.delta.y / hourHeight * 60) / 15) * 15;
      const dayShift = Math.floor(total / 1440);
      const withinDay = ((total % 1440) + 1440) % 1440;
      nextDate = shiftDate(destination, dayShift);
      nextTime = `${String(Math.floor(withinDay / 60)).padStart(2, "0")}:${String(Math.floor(withinDay % 60)).padStart(2, "0")}`;
    } else if (mode === "week") {
      const column = event.over?.rect;
      const dragged = event.active.rect.current.translated;
      if (column) {
        const centerY = dragged ? dragged.top + dragged.height / 2 : column.top + column.height / 2;
        const minutes = Math.max(0, Math.min(23 * 60 + 45, Math.round(((centerY - column.top) / hourHeight * 60) / 15) * 15));
        nextTime = `${String(Math.floor(minutes / 60)).padStart(2, "0")}:${String(minutes % 60).padStart(2, "0")}`;
      }
    }
    if (isFlexible) { await onPlaceFlexible(item, nextDate, nextTime ?? item.placementTime ?? "09:00"); return; }
    if (!original || original.date !== nextDate || nextTime && original.time !== nextTime) await onMove(item, nextDate, nextTime);
  }
  ...
  useEffect(() => {
    function navigateWithArrow(event: KeyboardEvent) {
      ...
      if (mode === "day" && event.code === "Space") { event.preventDefault(); setUnscheduledOpen((open) => !open); return; }
  ...
  return <div className="relative flex min-h-0 flex-1 flex-col overflow-hidden rounded-xl border bg-white shadow-sm dark:bg-neutral-900">
    ...
    {mode === "month" ? <><div className="grid grid-cols-7 border-b bg-stone-50 text-center text-[10px] font-semibold uppercase tracking-wider text-stone-500 dark:bg-neutral-800 dark:text-stone-300 sm:text-xs">{["Sun", "Mon", "Tue", "Wed", "Thu", "Fri", "Sat"].map((day) => <div key={day} className="py-2">{day}</div>)}</div><DndContext sensors={sensors} onDragStart={handleDragStart} onDragCancel={finishDrag} onDragEnd={(event) => { void handleDragEnd(event); }}><div className="min-h-0 flex-1 overflow-y-auto">{weekSegments.map(({ days, segments, laneCount }) => <div key={days[0]} className="relative grid grid-cols-7">{days.map((date) => <DayCell key={date} date={date} currentMonth={currentMonth} items={[...(byDay.get(date) ?? []), ...flexibleItems(date)]} timezone={trip.timezone} typeColors={typeColors} reservedRows={laneCount} selectedIds={selectedIds} primarySelectedId={primarySelectedId} onSelect={selectFromClick} onCreateItem={onCreateItem} />)}{segments.map((segment) => <MultiDayBar key={`${segment.item.id}:${days[0]}`} item={segment.item} weekStart={days[0]} startColumn={segment.startColumn} dayCount={segment.dayCount} lane={segment.lane} startsHere={segment.startsHere} endsHere={segment.endsHere} color={typeColor(segment.item.type, typeColors)} selected={selectedIds.has(segment.item.id)} primary={primarySelectedId === segment.item.id} onSelect={selectFromClick} />)}</div>)}</div><UnscheduledDrawer items={unscheduledItems} typeColors={typeColors} selectedIds={selectedIds} primarySelectedId={primarySelectedId} onSelect={selectFromClick} onAdd={() => onCreateItem()} /><DragOverlay dropAnimation={null}>{activeDragItems.length ? <CalendarDragPreview items={activeDragItems} timezone={trip.timezone} typeColors={typeColors} /> : null}</DragOverlay></DndContext></> : mode === "day" ? <DndContext sensors={sensors} onDragStart={handleDragStart} onDragCancel={finishDrag} onDragEnd={(event) => { void handleDragEnd(event); }}><DayAgenda date={focusDate} items={items} trip={trip} typeColors={typeColors} selectedIds={selectedIds} primarySelectedId={primarySelectedId} onSelect={selectFromClick} onCreateItem={(date, order) => onCreateItem(date, undefined, undefined, "(untitled event)", order)} /><UnscheduledDrawer items={unscheduledItems} typeColors={typeColors} selectedIds={selectedIds} primarySelectedId={primarySelectedId} onSelect={selectFromClick} onAdd={() => onCreateItem()} expanded={unscheduledOpen} onExpandedChange={setUnscheduledOpen} /><DragOverlay dropAnimation={null}>{activeDragItems.length ? <CalendarDragPreview items={activeDragItems} timezone={trip.timezone} typeColors={typeColors} /> : null}</DragOverlay></DndContext> : <DndContext sensors={sensors} onDragStart={handleDragStart} onDragCancel={finishDrag} onDragEnd={(event) => { void handleDragEnd(event); }}><div className="min-h-0 flex-1 overflow-auto"><div className="relative min-h-full min-w-[1050px]" onPointerDown={(event) => { if ((event.target as HTMLElement).closest("[role=button],button")) return; setZoomDrag({ x: event.clientX, y: event.clientY, hourHeight, dayWidth }); event.currentTarget.setPointerCapture(event.pointerId); }} onPointerMove={(event) => { if (!zoomDrag) return; const deltaX = event.clientX - zoomDrag.x; const deltaY = event.clientY - zoomDrag.y; setHourHeight(Math.min(110, Math.max(28, zoomDrag.hourHeight + deltaY * 0.35))); setDayWidth(Math.min(240, Math.max(90, zoomDrag.dayWidth + deltaX * 0.35))); }} onPointerUp={() => setZoomDrag(null)} onPointerCancel={() => setZoomDrag(null)}><div className="sticky top-0 z-30 bg-white shadow-sm dark:bg-neutral-900"><div className="flex border-b"><div className="sticky left-0 z-40 h-14 w-12 shrink-0 border-r bg-white dark:bg-neutral-900" />{weekDates.map((date) => <div key={date} className="h-14 shrink-0 border-r" style={{ width: dayWidth }}><p className="pt-1 text-center text-xs font-medium"><span className={date === today ? "rounded-full bg-emerald-700 px-2 py-1 text-white" : undefined}>{formatTripDate(`${date}T12:00:00Z`, trip.timezone, { weekday: "short", month: "short", day: "numeric" })}</span></p></div>)}</div></div><div className="flex"><div className="sticky left-0 z-20 w-12 shrink-0 bg-white dark:bg-neutral-900">{Array.from({ length: 24 }, (_, hour) => <div key={hour} className="relative border-b text-right text-[9px] text-muted-foreground" style={{ height: hourHeight }}><span className="absolute -top-2 right-1">{String(hour).padStart(2, "0")}:00</span></div>)}</div>{weekDates.map((date) => <ScheduleColumn key={date} date={date} items={scheduleItems(date)} allDayItems={topItemsForDate(date)} timezone={trip.timezone} typeColors={typeColors} width={dayWidth} hourHeight={hourHeight} selectedIds={selectedIds} primarySelectedId={primarySelectedId} onSelect={selectFromClick} onCreateItem={onCreateItem} onResize={onResize} onResizeStart={onResizeStart} />)}</div></div></div><UnscheduledDrawer items={unscheduledItems} typeColors={typeColors} selectedIds={selectedIds} primarySelectedId={primarySelectedId} onSelect={selectFromClick} onAdd={() => onCreateItem()} /><DragOverlay dropAnimation={null}>{activeDragItems.length ? <CalendarDragPreview items={activeDragItems} timezone={trip.timezone} typeColors={typeColors} /> : null}</DragOverlay></DndContext>}
    <div className="flex items-center gap-2 border-t px-4 py-2 text-xs text-muted-foreground"><Clock size={13} /> {mode === "day" ? "Hover between places to add an item. Drag flexible places to reorder them." : "Drag items to reschedule. Double-click an empty time to add an item. Drag empty space horizontally to change day width or vertically to change the time scale."}</div>
  </div>;
}
