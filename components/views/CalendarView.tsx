...
function ScheduleEvent({ item, date, timezone, color, hourHeight, narrowerOverlap, onSelect, onResize, onResizeStart }: { item: TravelObject; date: string; timezone: string; color: string; hourHeight: number; narrowerOverlap: boolean; onSelect: (item: TravelObject) => void; onResize: (item: TravelObject, endDate: string, endTime: string) => void; onResizeStart: (item: TravelObject, startDate: string, startTime: string) => void }) {
  ...
  return <div ref={setNodeRef} {...attributes} {...listeners} role="button" tabIndex={0} onClick={(event) => { event.stopPropagation(); onSelect(item); }} onDoubleClick={(event) => event.stopPropagation()} className={`group absolute z-[1] mx-1 w-[calc(100%-8px)] overflow-hidden rounded-md px-1.5 py-1 text-left text-[10px] leading-tight shadow-sm ${isDragging ? "opacity-40" : ""}`} style={style} title={`${item.title} · ${start.time}–${end.time}`}>{resizeHandle("start")}<span className="block truncate font-semibold">{item.title}</span><span className="block truncate opacity-80">{start.time}–{end.time}</span>{resizeHandle("end")}</div>;
}
...
function UnscheduledDrawer({ items, typeColors, onSelect, onAdd }: { items: TravelObject[]; typeColors: Record<string, string>; onSelect: (item: TravelObject) => void; onAdd: () => void }) {
  ...
  return <section onMouseEnter={openDrawer} onMouseLeave={closeDrawer} className="absolute inset-x-0 bottom-0 z-50">
    <div className={`grid overflow-hidden transition-[grid-template-rows] duration-200 ease-out ${open ? "grid-rows-[auto_1fr]" : "grid-rows-[auto_0fr]"}`}>
      <button type="button" aria-expanded={open} onFocus={openDrawer} onBlur={(event) => { if (!event.currentTarget.parentElement?.contains(event.relatedTarget)) closeDrawer(); }} className={`flex items-center gap-2 border border-b-0 bg-stone-100 px-4 py-2.5 text-left text-xs font-semibold shadow-xl transition-[width,border-radius] duration-200 dark:bg-neutral-800 ${open ? "w-full rounded-t-xl" : "mx-auto w-fit rounded-t-xl"}`}><ChevronDown size={14} className={`transition-transform ${open ? "rotate-180" : ""}`} />Unscheduled <span className="rounded-full bg-stone-200 px-2 py-0.5 text-[10px] dark:bg-neutral-700">{items.length}</span>{open && <span className="ml-auto text-[10px] font-normal text-muted-foreground">Drag an idea onto a date</span>}</button>
      <div className="min-h-0 overflow-hidden"><div className="flex w-full items-center gap-2 overflow-x-auto border-x border-t bg-stone-100 px-3 pb-3 pt-2 shadow-xl dark:border-neutral-700 dark:bg-neutral-800">{items.map((item) => <UnscheduledCard key={item.id} item={item} color={typeColor(item.type, typeColors)} onSelect={onSelect} />)}<Button variant="outline" size="sm" className="h-9 shrink-0" onClick={onAdd}>+ Add idea</Button>{items.length === 0 && <span className="text-xs text-muted-foreground">No unscheduled ideas yet.</span>}</div></div>
    </div>
  </section>;
}
...
function ScheduleColumn({ date, items, allDayItems, timezone, typeColors, width, hourHeight, onSelect, onCreateItem, onResize, onResizeStart }: { date: string; items: TravelObject[]; allDayItems: TravelObject[]; timezone: string; typeColors: Record<string, string>; width: number; hourHeight: number; onSelect: (item: TravelObject) => void; onCreateItem: (date: string, time: string) => void; onResize: (item: TravelObject, endDate: string, endTime: string) => void; onResizeStart: (item: TravelObject, startDate: string, startTime: string) => void }) {
  ...
  return <div ref={setNodeRef} className={`relative shrink-0 border-r ${isOver ? "bg-emerald-50/70 dark:bg-emerald-950/30" : ""}`} style={{ width }}>
    <div className="sticky top-14 z-20 h-0 overflow-visible">{allDayItems.map((item, index) => <div key={item.id} className="absolute inset-x-1 h-6" style={{ top: index * 28 }}><ScheduleStripItem item={item} date={date} color={typeColor(item.type, typeColors)} onSelect={onSelect} /></div>)}</div>
