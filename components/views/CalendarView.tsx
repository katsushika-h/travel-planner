...
function UnscheduledDrawer({ items, typeColors, onSelect, onAdd }: { items: TravelObject[]; typeColors: Record<string, string>; onSelect: (item: TravelObject) => void; onAdd: () => void }) {
  ...
  return <section onMouseEnter={() => setOpen(true)} onMouseLeave={() => setOpen(false)} className={`absolute bottom-2 left-1/2 z-50 -translate-x-1/2 overflow-hidden rounded-t-xl border bg-white shadow-xl transition-all dark:bg-neutral-900 ${open ? "w-[min(760px,95%)]" : "w-40"}`}>
    <button type="button" aria-expanded={open} onClick={() => setOpen((value) => !value)} className={`flex w-full items-center gap-2 text-left text-xs font-semibold ${open ? "px-4 py-2.5" : "justify-center px-3 py-2"}`}><ChevronDown size={14} className={`transition-transform ${open ? "rotate-180" : ""}`} />Unscheduled <span className="rounded-full bg-stone-200 px-2 py-0.5 text-[10px] dark:bg-neutral-800">{items.length}</span>{open && <span className="ml-auto text-[10px] font-normal text-muted-foreground">Drag an idea onto a date</span>}</button>
    {open && <div className="flex items-center gap-2 overflow-x-auto border-t bg-stone-50 px-3 pb-3 pt-2 dark:bg-neutral-950">{items.map((item) => <UnscheduledCard key={item.id} item={item} color={typeColor(item.type, typeColors)} onSelect={onSelect} />)}<Button variant="outline" size="sm" className="h-9 shrink-0" onClick={onAdd}>+ Add idea</Button>{items.length === 0 && <span className="text-xs text-muted-foreground">No unscheduled ideas yet.</span>}</div>}
  </section>;
}
