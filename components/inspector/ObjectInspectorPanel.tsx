...
export function ObjectInspectorPanel({ item, timeZone, tripStartDate, eventTypes, typeColors, darkMode, onClose, onChange, onDelete }: { item: TravelObject | null; timeZone: string; tripStartDate: string; darkMode: boolean; eventTypes: string[]; typeColors: Record<string, string>; onClose: () => void; onChange: (id: string, patch: Partial<TravelObject>, immediate?: boolean) => void; onDelete: (id: string) => Promise<void> }) {
  ...
  const start = dateParts(item.startDateTime, timeZone); const end = dateParts(item.endDateTime, timeZone);
