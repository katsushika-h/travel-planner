...
5. `lib/google-maps-csv.ts` parses Google Maps CSV rows and projects schedules without network calls. `lib/google-maps-import.ts` resolves Maps URLs and creates objects while preserving coordinate-free links. `components/trip/ImportGoogleMapsCsvButton.tsx` owns file selection and progress feedback.
