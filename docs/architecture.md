...
3. Schedule values are persisted and exposed as a date range, optional confirmed start/end times, optional flexible `placementTime`, and `dayOrder`. `lib/date-utils.ts` handles trip-timezone conversions; `lib/schedule-order.ts` defines pure ordering rules, and `lib/schedule-order-service.ts` writes per-date order inside API transactions. `lib/api-schedule.ts` parses canonical schedule requests; `lib/travel-object-serialization.ts` returns date-only API fields. The removed timestamp/day-index API keys are rejected on writes. See [ADR 0001](decisions/0001-canonical-schedule-fields.md) and [ADR 0002](decisions/0002-canonical-schedule-api.md).
...
5. `lib/google-maps-csv.ts` parses Google Maps CSV rows and projects schedules without network calls. `lib/google-maps-import.ts` resolves Maps URLs, creates objects, and returns per-row create outcomes while preserving coordinate-free links. `components/trip/ImportGoogleMapsCsvButton.tsx` owns file selection and progress feedback.
...
- Further changes to flexible-item ordering should use the existing database-backed ordering fixture and browser drag checks.
